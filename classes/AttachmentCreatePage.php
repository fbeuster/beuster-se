<?php
  ...
  class AttachmentCreatePage extends AbstractAdminPage {
      ...
      if ('POST' == $_SERVER['REQUEST_METHOD']) {
        ...
        if ($license == '') {
          ...
          $this->errors['license'] = array(
            'message' => I18n::t('admin.attachment.new.error.empty_license'),
            'value'   => $license);

        } else if (mb_strlen($license, 'UTF-8') > self::MAX_LENGTH_LICENSE) {
          # license too long
          $this->errors['license'] = array(
            'message' => I18n::t('admin.attachment.new.error.license_too_long',
        ...
        if ($name == '') {
          ...
          $this->errors['name'] = array(
            'message' => I18n::t('admin.attachment.new.error.empty_name'),
            'value'   => $name);

        } else if (mb_strlen($name, 'UTF-8') > self::MAX_LENGTH_NAME) {
          # name too long
          $this->errors['name'] = array(
            'message' => I18n::t('admin.attachment.new.error.name_too_long',
        ...
        if ($version == '') {
          ...
          $this->errors['version'] = array(
            'message' => I18n::t('admin.attachment.new.error.empty_version'),
            'value'   => $version);

        } else if (mb_strlen($version, 'UTF-8') > self::MAX_LENGTH_VERSION) {
          # version too long
          $this->errors['version'] = array(
            'message' => I18n::t('admin.attachment.new.error.version_too_long',
        ...
        if (!$has_uploads) {
          ...
          $this->errors['file'] = array(
            'message' => I18n::t('admin.attachment.new.error.no_file'),
            'value'   => '');
        }
        ...
        if (empty($this->errors)) {
          ...
          foreach ($_FILES['file']['name'] as $key => $value) {
            if (!File::isValidSize($_FILES['file']['size'][$key])) {
              # invlaid file size
              $this->errors[] = array(
                'message' => I18n::t( 'admin.attachment.new.error.invalid_file_size',
              ...
              if ($path === false) {
                # saving error
                $this->errors[] = array(
                  'message' => I18n::t('admin.attachment.new.error.file_save_failure'),
                  'value'   => $_FILES['file']['name'][$key]);
        ...
        if (empty($this->errors)) {
          $this->showMessage( I18n::t('admin.attachment.new.success'),
                              'admin');
        }
      }
    }
    ...
    private function load() {
      $this->setTitle(I18n::t('admin.attachment.new.label'));
    }
